...
from tools import ewma, getSoup, getStats

st.set_page_config(
    page_title="Lux AI stats",
    page_icon="🤖",
    layout="wide",
    menu_items={"About": "Submission statistics for Lux AI competition. Made by Yalikesifulei from Team ♂ GARCH ♂."},
)
...
if col2.button("Get stats", key="run"):
    ...
    if is_ok:
        ...
        fig.add_trace(go.Scatter(x=np.arange(1, len(scores) + 1), y=scores, name="scores", line={"color": "#1f77b4"}))
        fig.add_trace(
            go.Scatter(
                mode="lines",
                x=[1, len(scores) + 1],
        ...
        fig = go.Figure(layout=layout)
        fig.add_trace(
            go.Scatter(x=np.arange(1, len(outcomes) + 1), y=outcomes, line={"color": "lightgray", "dash": "dash"}, showlegend=False)
        )
        fig.add_trace(
            go.Scatter(
                mode="markers",
                x=np.argwhere(outcomes == 1).flatten() + 1,
                y=outcomes[outcomes == 1],
        ...
        fig.add_trace(
            go.Scatter(
                mode="markers",
                x=np.argwhere(outcomes == 0).flatten() + 1,
                y=outcomes[outcomes == 0],
        ...
        fig.add_trace(
            go.Scatter(
                mode="markers",
                x=np.argwhere(outcomes == 0.5).flatten() + 1,
                y=outcomes[outcomes == 0.5],
        ...
        fig = go.Figure(layout=layout)
        fig.add_trace(go.Scatter(x=np.arange(1, len(scores_delta) + 1), y=scores_delta, showlegend=False, line={"color": "#1f77b4"}))
        fig.add_trace(
            go.Scatter(
                mode="markers",
                x=np.argwhere(scores_delta > 0).flatten() + 1,
                y=scores_delta[scores_delta > 0],
        ...
        fig.add_trace(go.Scatter(x=np.arange(1, len(scores_delta) + 1), y=wr_change, name="win rate", line={"color": "#1f77b4"}))
        fig.data[-1].visible = True

        fig.add_trace(
            go.Scatter(
                mode="lines", x=[1, len(scores) + 1], y=[np.mean(outcomes)] * 2, name="current winrate", line={"color": "lightsalmon"}
            )
        )
        fig.data[-1].visible = True
